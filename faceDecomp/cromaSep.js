var colorShader = function () {
	...
	var fs = [
		...
		"void main() {",
			...
			"vec2 offsetR = vec2(vUv.x + (0.01*uv.x), vUv.y + (0.01*uv.y));",
			"vec2 offsetG = vec2(vUv.x + (0.005*uv.x), vUv.y + (0.005*uv.y));",
			"vec3 color;",
			"color.r = texture2D(texture, offsetR).r;",
			"color.g = texture2D(texture, offsetG).g;",
			"color.b = texture2D(texture, vUv).b;",
